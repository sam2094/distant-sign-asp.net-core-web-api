...
{
    public abstract class Logic<TInput, TOutput>
        where TOutput : LogicOutput, new()
    {
        ...
        public Logic(IUnitOfWork uow, string firstExecutedLogicName, bool beginTransaction = false)
        {
            _uow = uow;
        ...
        /// <returns></returns>
        public LogicResult<TOutput> Execute(TInput parameters)
        {
        ...
        /// <returns></returns>
        public async Task<LogicResult<TOutput>> ExecuteAsync(TInput parameters)
        {
            Parameters = parameters;
